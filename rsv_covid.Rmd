...
cols <- c(rep('gray',4),rgb(1,0,0,0.3),'blue','blue')
linetypes=c(rep(1,5),2,2)
select.counties <- grep('FL', dimnames(d1.c.alt)[[3]] )
...
lapply(select.counties, function(x){
  matplot(t(d1.c.alt[,,x,'rsv']), type='l', col=cols, bty='l', xlab='', ylab='', xlim=c(1,52), main=dimnames(d1.c.alt)[[3]][x], lty=linetypes)
abline(v=36, lty=2, col='gray')
})
