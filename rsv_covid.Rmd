...
last.wk <- max(d1.m.alt$epiwk[d1.m.alt$epiyr==2021] )
...
metro.codes <- unique(countries$sub_code[substr(countries$sub_code, 1,2) == "US"])
metro.codes <- metro.codes[!is.na(metro.codes)]
metro.codes <- metro.codes[71:length(metro.codes)]


metro.trends <- pblapply(metro.codes, function(x) {
    lapply(searches, function(y){
        gtrends(keyword='rsv', geo = c(x), category = 0, gprop='web',onlyInterest = T)
    })
  }
)

  test <- gtrends(keyword='rsv', geo = c('US-CT-YANTIC'), category = 0, gprop='web',low_search_volume=T,onlyInterest = T)
